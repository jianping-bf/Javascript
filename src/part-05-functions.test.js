...
test("all JS functions return something, but what happens if it's not explicit?", () => {
  ...
  expect(conditionalReturn(false)).toBe(undefined);
  // todo ...
});
...
test("calculator excerise", () => {
  const plus = (i, j) => i + j;
  const minus = (i, j) => i - j;
  const multiply = (i, j) => i * j;
  const divide = (i, j) => i / j;

  const calculate = ( func, i, j) => {
    return func(i, j);
  };
  expect(calculate(plus, 3, 4)).toBe(7);
  expect(calculate(minus, 3, 4)).toBe(-1);
